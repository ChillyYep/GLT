...
#endif // _DEBUG
Allocator::Allocator(uint32_t pageSize, uint32_t dataSize, uint32_t alignmentSize)
{
...
void Allocator::Reset(uint32_t pageSize, uint32_t dataSize, uint32_t alignment)
{
	FreeAll();
	...
	uint32_t minimal_size = (sizeof(BlockHeader) > m_szDataSize) ? sizeof(BlockHeader) : m_szDataSize;
